<?php
/*
Plugin Name: Smallerik File Browser for EAGLE
...
function wpfib_can_display_repo(&$error_text)
{
	...
	else
	{
		$error_text = "<div id='JS_MAIN_DIV'><div id='JS_ERROR_DIV'><table><tr>"
				...
				.__('Area reserved to the Project Consortium<br /><b>Users who need to access to this Reserved Area, please contact the <a href="mailto: buono@promoter.it">Webmaster</a> to ask for login information!</b>', 'wpfib')."</td>"
					 ."</tr></table></div><br /></div>";
		return false;
	}
}
...
function wpfib_render_files_div(&$text, $repo_abspath, $curdir_relpath, &$dirs, &$files)
{
	...
	$text .= "<table style='table-layout: fixed; word-wrap: break-word'>"
	...
	if(strlen($curdir_relpath))
	{
		...
		$text .= "<td class='fileIcon' style='width: 32px'>"
	...
	{
		$text .= "<td class='emptyTd' style='width: 56px'></td>";
	}
	...
	else
	{
		// CELL 2 (filename | header row)
           $text .= "<td class='fileName' style='width: 180px'>"
		...
		{
			$text .= "<td class='fileSize' style='width: 100px'>"
		...
		{
			$text .= "<td class='emptyTd' style='width: 100px'></td>";
		}

		// CELL 4 (filedate | header row)
		if ($wpfib_options['display_filedate'] && count($files))
		{
			$text .= "<td class='fileChanged' style='width: 100px'>"
        ...
        $text .= "<td colspan='2' class='emptyTd' style='width: 60px'></td>";
	}
	$text .= "</tr>";
...
//			if ($is_dir_in_webroot && wpfib_makeThumbnail($a_file["name"], $a_file["extension"], $dir, $wpfib_options['thumbsize'], $wpfib_options['thumbsize']))
			...
			else
			{
				...
				if ($is_dir_in_webroot && ($wpfib_options['type_of_link_to_files'] != LINK_THROUGH_SCRIPT))
				{
					...
					$dir_relpath_mod = substr ( $dir_relpath , strpos ( $dir_relpath , '/repository' ) );
					$file_link_a_tag_begin = "<a href=\"".$dir_relpath_mod.DS.$a_file["name"]."\" "
...
//			$text .= "<form name='uploadForm' style='display:inline; margin: 0px; padding: 0px;' enctype='multipart/form-data' noaction='".$wpfib_baselink."&dir=".urlencode($curdir_relpath)."' method='post'>";
			...
			//	."	</table>"
				...
				."<tr>"
				."	<td colspan=\"3\">"
				."	The maximum file size is 32M. To upload bigger files please contact Promoter team."
				."	</td>"
				."</tr>";
		}
